...
extension $TestStateUnion on TestState {
  T when<T>({
    ...
    required T Function({required int counter, required bool isLoading, String? errorMessage}) loaded,
  }) => switch (this) {
    TestStateInitial _ => initial(),
    TestStateLoading _ => loading(),
    TestStateLoaded v => loaded(counter: v.counter, isLoading: v.isLoading, errorMessage: v.errorMessage),
  };

  T maybeWhen<T>({
    _$TestStateSig0<T>? initial,
    _$TestStateSig0<T>? loading,
    T Function({required int counter, required bool isLoading, String? errorMessage})? loaded,
    required T Function() orElse,
  }) => switch (this) {
    TestStateInitial _ when initial != null => initial(),
    TestStateLoading _ when loading != null => loading(),
    TestStateLoaded v when loaded != null => loaded(counter: v.counter, isLoading: v.isLoading, errorMessage: v.errorMessage),
    _ => orElse(),
  };
  ...
  TestStateInitial? get asInitial => this is TestStateInitial ? this as TestStateInitial : null;
  TestStateLoading? get asLoading => this is TestStateLoading ? this as TestStateLoading : null;
  TestStateLoaded? get asLoaded => this is TestStateLoaded ? this as TestStateLoaded : null;
}
...
class TestStateLoaded extends TestState {
  ...
  @override
  bool operator ==(Object other) =>
    identical(this, other) || other is TestStateLoaded && counter == other.counter && isLoading == other.isLoading && errorMessage == other.errorMessage;
